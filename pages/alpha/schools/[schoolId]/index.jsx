...
export async function getServerSideProps({ params }) {
  ...
  const teachers = await sql`
    SELECT 
      ...
      SUM(
        CASE 
          WHEN s.message_status = 'not_sent' OR s.message_status IS NULL 
          THEN 1 
          ELSE 0 
        END
      ) AS pending_count
    FROM alpha_teachers t
    LEFT JOIN alpha_students s 
      ON s.teacher_id = t.id
    WHERE t.school_id::text = ${schoolId}
...
export default function AlphaTeachersPage({
  teachers,
  schoolId,
  schoolName,
}) {
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-6">
      <div className="max-w-5xl mx-auto">
        <div className="mb-6">
          <Link href="/alpha" className="text-sm text-green-600 font-medium">
            ← Back to Schools
          </Link>
        ...
        {teachers.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm border p-8 text-center">
            ...
            <p className="text-sm text-gray-500 mt-2">
              This school has no teachers linked in alpha_teachers.
            </p>
          </div>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {teachers.map((t) => {
              ...
              return (
                <Link
                  key={t.id}
                  href={`/alpha/schools/${schoolId}/teachers/${t.id}`}
